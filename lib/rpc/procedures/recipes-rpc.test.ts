...
import { parseRecipeVramJson, ExtendedRecipeSchema } from "./recipes";
import { readFile } from "node:fs/promises";
import { join } from "node:path";

const FIX = join(__dirname, "../../__fixtures__");

describe("recipes RPC procedures (SC-P0-39, SC-P0-40) — unit tests", () => {
  ...
  describe("ExtendedRecipeSchema (SC-P0-39)", () => {
    it("validates a recipe with vram data", () => {
      const recipe = {
        name: "@test/recipe",
        file: "/tmp/recipe.yaml",
        path: "/tmp/recipe.yaml",
        model: "test-model",
        runtime: "vllm",
        min_nodes: 1,
        registry: "official",
      ...
      const result = ExtendedRecipeSchema.parse(recipe);
      expect(result.name).toBe("@test/recipe");
      expect(result.vram?.fits_dgx_spark).toBe(true);
    });

    it("validates a recipe without vram data", () => {
      const recipe = {
        name: "@test/recipe",
        file: "/tmp/recipe.yaml",
        path: "/tmp/recipe.yaml",
