...
/** Infer the model's specialty category from its name/description. */
function inferModelCategory(model: string, description: string = ""): string {
  const text = `${model} ${description}`.toLowerCase();

  // Image / diffusion generation
  if (/diffusion|image.?gen|stable.?diffusion|sdxl|flux/.test(text)) return "diffusion";
...
const CATEGORIES = ["coding", "reasoning", "vision", "diffusion", "general"];
...
/** Extended recipe entry with per-GPU VRAM estimation. */
export const ExtendedRecipeSchema = z
  .object({
    name: z.string(),
    file: z.string(),
    path: z.string(),
    ...
    vramError: z.string().nullable().optional(),
  })
  .loose();

/** Recipe list enriched with inferred model category. */
const ExtendedRecipeListItemSchema = z.array(
...
/** List recipes with inferred model category (coding, reasoning, vision, diffusion, general). */
export const listWithCategory = os
  .input(z.object({ all: z.boolean().default(true) }).optional())
  .output(ExtendedRecipeListItemSchema)
  .handler(async ({ input }) => {
    const args = ["list", "--json"];
    if (input?.all !== false) args.push("--all");
    const raw = await runSparkrunJson<unknown>(args);
    const recipes = RecipeListSchema.parse(raw);
    return recipes.map((r) => ({
      ...r,
      category: inferModelCategory(r.model, r.description),
    }));
  });

/** Fetch all recipes with VRAM estimation — suitable for client-side filtering. */
const MAX_VRAM_CONCURRENCY = 4;
const VRAM_TIMEOUT_MS = 15000; // 15s per recipe

async function collectVram(
  recipeNames: string[],
  signal?: AbortSignal,
  ...
  async function fetchOne(name: string): Promise<void> {
    ...
    try {
      const r = await runSparkrun(["recipe", "vram", name, "--json"], {
        timeoutMs: VRAM_TIMEOUT_MS,
        signal,
      });
      if (r.code === 0 && r.stdout.trim()) {
        const raw = JSON.parse(r.stdout);
        results[name] = { vram: parseRecipeVramJson(raw), vramError: null };
      } else {
        results[name] = { vram: null, vramError: r.stderr.trim() || "vram not available" };
      }
    } catch {
      results[name] = { vram: null, vramError: "vram fetch failed" };
    }
  }

  // Process in batches to avoid OOM and timeout
  for (let i = 0; i < recipeNames.length; i += MAX_VRAM_CONCURRENCY) {
    if (signal?.aborted) break;
    const batch = recipeNames.slice(i, i + MAX_VRAM_CONCURRENCY);
...
export const listExtended = os
  .input(z.object({ all: z.boolean().default(true) }).optional())
  .output(z.array(ExtendedRecipeSchema))
  .handler(async ({ input, signal }) => {
    const listArgs = ["list", "--json"];
    if (input?.all !== false) listArgs.push("--all");
    const raw = await runSparkrunJson<unknown>(listArgs);
    const recipes = RecipeListSchema.parse(raw) as RecipeListItem[];
